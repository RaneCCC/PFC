...
演化复幅度A_j，重建密度场ψ
clang++ -O3 -o apfc apfc.cpp -std=c++17 -I../../FFT/include -L../../FFT/lib -lfftw3
*/
...
// 宏定义，简化多维数组访问（参考PFC_CUDA） 但是没有用上 因为用的地方比较少
#define A_R_macro v.A_R[0], v.A_R[1], v.A_R[2]
#define nl_R_macro v.nl_R[0], v.nl_R[1], v.nl_R[2]
        ...
        for (int j = 0; j < N; ++j) {
            ...
            for (const Seed& s : seeds) {
                ...
                if (distance < s.radius) {
                    ...
                    A1_R[idx] = A0 * exp(z(0, dot_product(k1_rot - k1, x_vec)));
                    A2_R[idx] = A0 * exp(z(0, dot_product(k2_rot - k2, x_vec)));
                    A3_R[idx] = A0 * exp(z(0, dot_product(k3_rot - k3, x_vec)));
                }
            }
        }
    }
...
void fftw_initialize() {
    ...
    for(int i = 0; i < 3; ++i)  v.A_k[i] = (fftw_z*) fftw_malloc(size_fftw_z * Size);
    for(int i = 0; i < 3; ++i)  v.nl_k[i] = (fftw_z*) fftw_malloc(size_fftw_z * Size);
...
vector<z> ifft_2d(fftw_z* input) {
    ...
    vector<z> output(Size);// 归一化
    for(int i = 0; i < Size; ++i) output[i] = z(v.out_z[i][0], v.out_z[i][1]) / static_cast<double>(Size);
...
void update_k(fftw_z* Aj, const fftw_z* nl, const z& k_j) {
    ...
    for (int i = 0; i < N; ++i) {
        ...
        for (int j = 0; j < N; ++j) {
            ...
            // 旋转协变 线性算子Lj
            double Lj = (dot_product(q1, q1)) * (Gamma - op_2 * op_2);
...
void reconstruct_output(int steps) {
    ...
    for (int i = 0; i < N; ++i) {
        for (int j = 0; j < N; ++j) {
            ...
            z A_sum = v.A_R[0][idx] * exp(z(0, dot_product(k1, x_vec))) +
                          v.A_R[1][idx] * exp(z(0, dot_product(k2, x_vec))) +
                          v.A_R[2][idx] * exp(z(0, dot_product(k3, x_vec)));
